...
<script>
    ...
    methods: {
      ...
      open(){
          if(this.formtitle == '新增参数'){
            ...
            RequestPost("/suyuanClasses/add",this.subData).then(response => {
	  ...
	  deleteRow(index, rows) {
       this.$confirm('确认删除, 是否继续?', '提示', {
            ...
            RequestPost("/suyuanClasses/delete",rows).then(response => {
	...
	loadData(){
		...
		RequestGet("/suyuanClasses/findAll",this.page).then(response => {
						if(response.code == '0000'){
