<template> 
 <div class="block">   
		...
		<el-table :data="datalist" highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="classes" label="单位名" width="250" sortable>
			</el-table-column>
        ...
        <el-dialog   :title="formtitle" :visible.sync="dialogFormVisible" >
			<el-form ref="subData" :model="subData" label-width="100px" @submit.prevent="onSubmit" style="margin:20px;">
                    <el-form-item label="*单位名">
                      <el-input v-model="subData.classes"  placeholder="请输入单位名"></el-input>
                    </el-form-item>
...
<script>
    ...
    methods: {
      ...
      open(){
          if(this.formtitle == '新增参数'){
            ...
            RequestPost("/suyuanWeight/add",this.subData).then(response => {
	  ...
	  deleteRow(index, rows) {
       this.$confirm('确认删除, 是否继续?', '提示', {
            ...
            RequestPost("/suyuanWeight/delete",rows).then(response => {
	...
	loadData(){
		...
		RequestGet("/suyuanWeight/findAll",this.page).then(response => {
						if(response.code == '0000'){
